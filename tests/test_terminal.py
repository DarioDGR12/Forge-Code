...
from forge_code.tools.terminal import load_cwd, load_terminal, shell_snapshot
...
def test_bash_peels_cd_and_records_last(tmp_path: Path) -> None:
    (tmp_path / "src").mkdir()
    (tmp_path / "src" / "ok.txt").write_text("hi\n", encoding="utf-8")
    tools = default_registry()
    listed = tools.execute(tmp_path, "bash", {"command": "cd src && ls"})
    assert listed.startswith("exit 0")
    assert "cwd src" in listed
    assert "ok.txt" in listed
    assert load_cwd(tmp_path).name == "src"
    run_bash(tmp_path, {"command": "false"})
    snap = shell_snapshot(tmp_path)
    assert snap["cwd"] == "src"
    assert snap["last_exit"] == "1"
    token = tools.execute(tmp_path, "bash", {"command": "echo sk-abcdefghijklmnopqrstuvwxyz"})
    assert "sk-abcdefghijklmnopqrstuvwxyz" in token
    assert "sk-abcdefghijklmnopqrstuvwxyz" not in load_terminal(tmp_path)
    assert "***" in load_terminal(tmp_path)
